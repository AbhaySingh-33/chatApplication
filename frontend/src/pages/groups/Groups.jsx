...
import useListenGroupEvents from "../../hooks/useListenGroupEvents";
    ...
    return (
        <div className="flex items-center justify-center p-4 h-full">
            <div className="flex h-[75vh] sm:h-[420px] md:h-[520px] lg:h-[580px] xl:h-[620px] rounded-2xl overflow-hidden bg-white/5 backdrop-blur-lg border border-white/10 shadow-2xl relative z-10 animate-slide-up w-[90%] sm:w-[90%] max-w-none sm:max-w-[950px]">
                ...
                {/* Sidebar */}
                <div
                    className={`${selectedGroup?._id ? "hidden sm:flex" : "flex"
                        } ${sidebarOpen ? "sm:w-[340px]" : "sm:w-0"} w-full h-full flex-col overflow-hidden animate-fade-in-left transition-all duration-300 ${!selectedGroup?._id ? "pt-14 sm:pt-0" : ""
                        }`}
                >
                    <GroupSidebar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
                </div>

                {/* MessageContainer */}
                <div
                    className={`${selectedGroup?._id ? "flex " : "hidden sm:flex"
                        } w-full flex-1 animate-fade-in-right`}
                >
                    <GroupMessageContainer sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
                </div>
            </div>
        </div>
    );
